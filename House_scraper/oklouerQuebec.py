...
class Oklouer:
    def __init__(self):
        ...
        page = self.session.get(
            f"{self.base_url}/logements/studios-lofts_1-1-2_2-1-2")
        ...
        while True:
            for loop in soup.find_all('div', class_="unit"):
                try:
                    address = loop.find(
                        'div', class_="col-xs-12 col-sm-6 unit-infos").find('h3').text.capitalize()
                ...
                if address not in address_list:
                    ...
                    try:
                        img = loop.find(
                            'img', class_="img-responsive full-width").get('src')
                    except AttributeError as ae:
                        continue
                    try:
                        maps = loop.find(
                            'div', class_="col-xs-12 col-sm-3 hidden-xs").find('img').get('src')
                    except AttributeError as ae:
                        continue
                    source_price = self.session.get(f"{self.base_url}{url}")
                    soup_price = BeautifulSoup(source_price.content, 'lxml')
                    try:
                        price = soup_price.find(
                            'div', class_="pull-right price").find('small').text.split(" ")[1].split("/")[0]
                    except AttributeError as ae:
                        continue
                    try:
                        info = soup_price.find(
                            'div', class_="col-xs-6 col-sm-6 infos text-left").text.rstrip().lstrip()
                    except AttributeError as ae:
                        continue
